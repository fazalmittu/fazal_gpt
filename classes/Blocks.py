...
from classes.FeedForward import FeedFoward
from classes.MultiHeadAttention import EDMultiHeadAttention, MultiHeadAttention
...
class DecoderBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        head_size = embedding_dim // n_heads
        self.sa_masked = EDMultiHeadAttention(n_heads, head_size, embedding_dim, block_size, dropout, True)
        self.sa = EDMultiHeadAttention(n_heads, head_size, embedding_dim, block_size, dropout, False)
        self.ffwd = FeedFoward(embedding_dim, dropout)
        self.ln1 = nn.LayerNorm(embedding_dim)
        self.ln2 = nn.LayerNorm(embedding_dim)
